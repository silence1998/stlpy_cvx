...
class SingleTrack(Model):
    ...
    def __init__(self, K, t_f_guess, spec, max_k, smin_C,
                 x_init, x_final,
        ...
        """
        Model.__init__(self, K, t_f_guess, max_k=max_k, smin_C=smin_C)
        ## dynamic parameters
        self.a_max = 11.4
        self.a = dynamic_model.parameters.a
        self.b = dynamic_model.parameters.b
        self.l_wb = self.a + self.b
        self.s_prime = cvx.Variable((self.K, 1), nonneg=True)

        self.x_init = x_init
        self.x_final = x_final
        self.fix_final_state = 0
        self.final_position_max = None
        self.final_position_min = None
    ...
    def settingFinalAngel(self, angel):
        self.angel = angel
    def settingStateBoundary(self, x_min, x_max):
        self.x_min = x_min
        self.x_max = x_max

    def settingControlBoundary(self, u_min, u_max):
        self.u_min = u_min
        self.u_max = u_max

    def settingWeights(self, u_weight, velocity_weight):
        self.u_weight = u_weight
        self.velocity_weight = velocity_weight

    def settingFixFinalState(self):
        self.fix_final_state = 1

    def settingFinalPositionLimit(self, max_, min_):
        self.final_position_max = max_
        self.final_position_min = min_
    ...
    def add_interpolate_function(self, spec, index, var):  ### flag = 0 before temp logical flag=1 after
        ...
        if spec.timesteps == list(range(self.K + 1)):
            if isinstance(spec.subformula_list[0], MovingNonlinearPredicate):
                ...
                tt = var[0:5] + var[tmp_position_index:tmp_position_index+self.center_var_number]
                ...
                self.f_a[index_, 0] = spec.subformula_list[0].g(var[0:5])
                self.f_b[index_, 0] = var[self.n_x + self.n_x_sub + index_, 0]
                ### TODO now 0,1 are manually setting, later use STL to get it.
            elif isinstance(spec.subformula_list[0], LinearPredicate):
                self.var_index.append([0, 1, self.n_x + self.n_x_sub + index_])
                self.var_lambda_index.append([0, 1, self.n_x + self.n_x_sub + index_])
                self.f_a[index_, 0] = spec.subformula_list[0].a.T @ var[0:5] - spec.subformula_list[0].b
                self.f_b[index_, 0] = var[self.n_x + self.n_x_sub + index_, 0]
            else:
        ...
        for i, subformula in enumerate(spec.subformula_list):
            if isinstance(subformula, MovingNonlinearPredicate):
                ...
                if not 4 in list_index:
                    list_index.append(4)
                    list_lambda_index.append(4)
                ...
                list_function.append(subformula.g(var[0:5]))
                if not 0 in list_index:
                    list_index.append(0)
                    list_lambda_index.append(0)
                if not 1 in list_index:
                ...
                tmp = subformula.a.T @ var[0:5] - subformula.b
    ...
    def get_equations(self):
        ...
        x = sp.Matrix(sp.symbols('x y d v p', real=True))
        u = sp.Matrix(sp.symbols('v_d a', real=True))

        f[0, 0] = x[3, 0]*sp.cos(x[4, 0])
        f[1, 0] = x[3, 0] * sp.sin(x[4, 0])
        f[2, 0] = u[0, 0]
        f[3, 0] = u[1, 0]
        f[4, 0] = x[3, 0]/self.l_wb*sp.tan(x[2, 0])
        ...
        self.f_c = sp.zeros(1, 1)
        self.f_c[0, 0] = sp.sqrt(u[1, 0] ** 2 + (x[3, 0] * x[3, 0] / self.l_wb * sp.tan(x[2, 0]))**2 + 1e-6)

        self.f_c_func = sp.lambdify((x, u), self.f_c, 'numpy')
        f_c_gradient_x = self.f_c.jacobian(x)
        f_c_gradient_u = self.f_c.jacobian(u)
        self.f_c_func_gradient_x = sp.lambdify((x, u), f_c_gradient_x, 'numpy')
        self.f_c_func_gradient_u = sp.lambdify((x, u), f_c_gradient_u, 'numpy')
    ...
    def get_objective(self, X_v, X_robust_v, U_v, X_last_p, X_robust_last_p, U_last_p):
        ...
        """
        return cvx.Minimize(0.1*(self.u_weight * cvx.sum_squares(U_v) - X_robust_v[-1, 0]) + 1e5 * cvx.sum(self.s_prime))
    ...
    def get_constraints(self, X_v, X_robust_v, U_v, X_last_p, X_robust_last_p, U_last_p):
        ...
        if self.angel is not None:
            constraints += [
                X_v[4, self.K - 1] >= self.angel[0],
                X_v[4, self.K - 1] <= self.angel[1]
            ]

        if self.fix_final_state:
            constraints += [
                X_v[:, self.K - 1] == self.x_final
            ]
        ...
        if self.final_position_max is not None:
            constraints += [
                X_v[0:2, -1] <= self.final_position_max,
                X_v[0:2, -1] >= self.final_position_min
            ]

        constraints += [X_robust_v[0, 0] >= 0]
        ...
        constraints += [
            # State constraints:
            X_v[2, :] <= math.pi/3, X_v[2, :] >= -math.pi/3#,
            #X_v[4, :] <= math.pi, X_v[4, :] >= -math.pi
        ]
        constraints += [
            U_v[0, :] <= 0.4, U_v[0, :] >= -0.4
        ]

        ###########consider the friction circle
        for k in range(self.K):
            constraints += [
                self.f_c_func(X_last_p.value[:, k], U_last_p.value[:, k]) + \
                self.f_c_func_gradient_x(X_last_p.value[:, k], U_last_p.value[:, k]) @ (X_v[:, k] - X_last_p.value[:, k]) + \
                self.f_c_func_gradient_u(X_last_p.value[:, k], U_last_p.value[:, k]) @ (U_v[:, k] - U_last_p.value[:, k]) - \
                self.a_max <= self.s_prime[k, 0]
            ]
    ...
    def get_nonlinear_cost(self, X=None, U=None):
        # magnitude = np.zeros((self.K, 1))
        # for k in range(self.K):
        #     magnitude = np.sqrt(U[1, k]**2 + (X[3, k]**2/self.l_wb*np.tan(X[2, k]))**2)
        magnitude = np.sqrt(U[1, :]**2 + (X[3, :]**2/self.l_wb*np.tan(X[2, :]))**2)
        is_violated = magnitude > self.a_max
        violation = magnitude - self.a_max
