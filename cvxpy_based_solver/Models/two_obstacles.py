...
class DoubleIntegral:
    ...
    def __init__(self, K, t_f_guess):
        ...
        self.x_final = np.array([10.0, 0.0, 0, 0])

        self.u_min = np.array([-10, -10])
        self.u_max = np.array([10, 10])
        self.x_min = np.array([-15.0, -1.8, -10, -10])
        self.x_max = np.array([15.0, 1.8, 10, 10])

        self.goal_center = (10, 0)
        self.goal_radius = 0.5

        self.obstacle_center = np.array([[3.5, 0.5], [7, -0.5]])
        self.obstacle_radius = np.array([1.0, 1.0])
        # Goal Reaching
        ...
        self.spec = not_at_obstacle_1.always(0, self.K) & not_at_obstacle_2.always(0, self.K) & at_goal.eventually(0, self.K)
