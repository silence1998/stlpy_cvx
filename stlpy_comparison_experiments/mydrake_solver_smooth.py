from stlpy.solvers.drake.drake_base import DrakeSTLSolver
from stlpy.STL import LinearPredicate, NonlinearPredicate
from stlpy.systems.nonlinear import NonlinearSystem
...
class MyDrakeSmoothSolver(DrakeSTLSolver):
    """
    ...
    def __init__(self, spec, sys, x0, T, k=2.0, verbose=True):
        ...
        if snopt_solver.available():
            ...
            self.solver = MakeSolver(IpoptSolver().solver_id())
        self.option = SolverOptions()
        self.option.SetOption(IpoptSolver().solver_id(), "max_iter", 3000)
    ...
    def AddDynamicsConstraints(self):
        ...
        if isinstance(self.sys, LinearSystem):
        ...
        elif isinstance(self.sys, NonlinearSystem):
            for t in range(self.T-1):
                self.mp.AddConstraint(eq(
    ...
    def AddGoalConstraints(self, goal):
        self.mp.AddConstraint(eq(
                    self.x[:, self.T-1], goal
                ))
    ...
    def Solve(self):
        ...
        st = time.time()
        res = self.solver.Solve(self.mp, initial_guess=initial_guess, solver_options=self.option)
