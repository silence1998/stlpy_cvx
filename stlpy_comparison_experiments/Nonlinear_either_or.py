...
class NonlinearReachAvoid(BenchmarkScenario):
    ...
    def GetSpecification(self):
        ...
        spec = not_at_obstacle_1.always(0, self.T) & not_at_obstacle_2.always(0, self.T) &\
              (at_goal_1|at_goal_2).eventually(0, self.T) & at_goal.eventually(0, self.T)
...
x_max = np.array([10.0, 10.0, 5.0, 5.0])
solver.AddControlBounds(u_min, u_max)
solver.AddStateBounds(x_min, x_max) # Set bounds on state and control variables

#solver = MyScipyGradientSolver(spec, sys, x0, T) # Scipy cannot set boundary
