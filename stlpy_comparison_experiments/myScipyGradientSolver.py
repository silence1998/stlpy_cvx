...
class MyScipyGradientSolver(STLSolver):
    """
    ...
    def Solve(self):
        ...
        option_ = dict()
        option_["maxiter"] =10000
        start_time = time.time()
        res = minimize(self.cost, u_guess.flatten(),
                method=self.method, options=option_)
    ...
    def forward_rollout(self, u):
        ...
        if isinstance(self.sys, LinearSystem):
            for t in range(T-1):
                x[:,t+1] = self.sys.A@x[:,t] + self.sys.B@u[:,t]
                y[:,t] = self.sys.C@x[:,t] + self.sys.D@u[:,t]

            y[:,T-1] = self.sys.C@x[:,T-1] + self.sys.D@u[:,T-1]
