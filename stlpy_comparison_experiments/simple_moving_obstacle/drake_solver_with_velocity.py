...
from linear_predicate_with_velocity import LinearPredicateWithVelocity
import numpy as np
import time
...
class DrakeMICPSolverWithVelocity(DrakeSTLSolver):
    """
    ...
    def AddSubformulaConstraints(self, formula, z, t):
        ...
        elif isinstance(formula, LinearPredicateWithVelocity):
            # a.T*y - b - v*t + (1-z)*M >= rho
            y = self.y[:,t]
            self.mp.AddLinearConstraint(ge(
                formula.a.T@y - formula.b - formula.v*t + (1-z)*self.M, self.rho
            ))
